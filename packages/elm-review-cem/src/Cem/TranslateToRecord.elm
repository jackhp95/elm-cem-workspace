...
`<root>.<Comp>.view attrs children` to the required-record form
`<root>.<Comp>.el { <requiredFields> } attrs children`, hoisting the required
fields out of the children/attrs per `Cem.Facts`.
...
Applies ONLY to the components that HAVE an `el`/required record (those whose
`facts` list the `Record` facet — 29 in elm-m3e). It is a clean no-op for every
other component.
...
Everything not hoisted stays verbatim in the `el` call's attrs/children lists.
...
a single-pass fixpoint — its output uses `el`, never `view`, so re-running the
...
## Facts gaps (blockers for the docs pipeline / package 2)
  ...
  - Components whose required record carries `ariaLabel : String` (`fab`,
    ...
    `aria-label` string has no setter on the Standard `view` surface, so the field
  ...
  - The `unnamed`-slot default setter (`child`), the record constructor (`el`) and
    the `<root>.Action` module/`none` constructor are treated as fixed library
    conventions (not carried in the facts), matching how `PreferBarrel` hardcodes
    `view`/aria names.
