...
value that identifies the FAB in YOUR brand's facts (elm-cem's noun for the tag
that plays the FAB role — `"fab"` for `m3e-fab`); the rest of the rule is
fully fact-driven (label slot setter name from `slotRewrites`, module/namespace
from the fact itself). This package holds no brand's noun hardcoded — the
consumer's own ReviewConfig supplies it, same pattern as `Cem.fences`'s
`brandRoots`/`seamModules` or `Cem.redundantElementEscape`'s `seamEscapes`.
-}
