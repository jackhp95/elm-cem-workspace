...
This is a DIFFERENT kind of "not emittable" from `ASkip`, and the distinction is
the whole point of a second predicate. `ASkip` is about the VALUE: a DOM element
reference or a callback has no Elm spelling, so no setter is possible. This is
about the NAME: the value is a perfectly ordinary string, and the setter compiles
and runs — it just does not do what it says, because `elm/virtual-dom`'s kernel
silently rewrites the name on the way to the DOM, or the DOM ignores it at the
point virtual-dom writes it.
...
`data-formaction="…"` — a `<button formaction>` that does not override the form's
action. The bug is invisible from Elm and invisible in the generated docs. So the
policy is: do not advertise it. Every emitted surface omits it (shared canonical,
per-element re-export, builder pipe, `Attrs` capability row, `Review/Facts`
roster), the run REPORTS each omission on the info channel, and the emitted
`<Lib>.Attributes` docs carry a note naming the kernel function responsible.
...
defeatable from Elm. If you need one of these attributes, the escape hatch is a
port or a custom element, not a setter here.
...
## The cases, and the kernel code that causes each
  ...
  - **Any content attribute whose name matches `/^(on|formAction$)/i`.**
    ...
    `on*` content attribute is in any manifest today, but a manifest refresh that
    adds one must not quietly ship a dead setter.

    Note how blunt that regex is. `^on` is a PREFIX with no boundary, so it also
    catches innocent names — `once`, `online`, `onward` would all render as
    `data-once` / `data-online` / `data-onward`. That is the kernel
  ...
  - **`formaction` specifically**, which is why the case above says "matches",
    not "starts with `on`". The regex's alternation is `(on|formAction$)` and the
    `i` flag applies to the whole pattern, so `^formAction$` matches the
    ...
    exact key `formAction`; and the lowercase key `formaction` escapes that test
    only to become an inert JS expando — `<button>` has no `formaction`
    property, so nothing observes the write. There is NO working path from Elm
    for `formaction`, in either form.

  - **`innerHTML` / `outerHTML` as PROPERTY keys.**
    `VirtualDom.property` runs
    ...
    `innerHtml` slips past the test and then fails the other way: the element's
    property is `innerHTML`, so `innerHtml` is an inert expando. Both spellings
    are matched here, case-insensitively, because both have the same outcome —
    ...
    writes a meaningless-but-untouched attribute. That is a manifest problem, not
    a kernel one, and this predicate answers only for the kernel.
  ...
  - **`is`, in either form.** Customized built-in elements are opted in at
    ...
    `createElementNS(vNode.__namespace, vNode.__tag)`, also two-argument). So by
    the time any attribute or property fact is applied the element already
    exists as its plain built-in self, and neither `setAttribute("is", …)` nor an
    `is` property write can upgrade it. `is` has no IDL attribute either, so the
    property form is an inert expando.
    ...
    asserts all 29 globals are expressible. It stays in that config's `_globals`
    roster — the config faithfully describes HTML — and the gate carries an
    explicit kernel-blocked allowance naming this reason, plus a check that the
    omission actually happened.


## Why this OMITS rather than fails the run

The family's habit is fail-loud (`guardHomeAttrTypes`, `guardHomeAttrForms`, the
config-decode error). Those all describe an AUTHORING mistake with a concrete fix
the author can apply: retype the attribute, rename it, split the home module, fix
the config typo. Failing is right because the run cannot produce a correct answer
and the author can.

A kernel-blocked attribute is not that. The manifest is CORRECT — `formaction`
and `is` are real HTML, faithfully described — and the thing that cannot express
them is `elm/virtual-dom`. There is no edit to the manifest or the config that
resolves it, so failing would mean every regeneration of a correct manifest
aborts forever. That is the wrong answer to "HTML has an attribute Elm cannot
write".

So: omit, and REPORT. One deterministic note per omission on the same info
channel the K2/K3 collapse notes use, naming the element, the attribute and the
kernel function — because silence is the failure mode this generator keeps having
to stamp out (the `datetime` regression, the `AEnumMap` degradation).
