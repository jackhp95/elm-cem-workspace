...
function measurePackage(elm, name, summary, ownSrc, exposed, vendorSrcs, deps) {
  ...
  delete externalDeps["jackhp95/elm-html-intermediate-representation"];
...
function runSplit(o, elm, cwd, packagesPath) {
  ...
  for (const p of packages) {
    ...
    for (const d of deps) {
      ...
      else if (d === "jackhp95/elm-html-intermediate-representation" && irSrc) vendorSrcs.push(irSrc);
    ...
    for (const d of Object.keys(p.deps || {})) {
      if (d === "jackhp95/elm-html-intermediate-representation" && irSrc && !vendorSrcs.includes(irSrc)) vendorSrcs.push(irSrc);
