...
// attrSeamFnOf -> the function name on the attr seam that spells a custom HTML
// attribute. Default "attribute" (historical); the real TypedHtml foundation
// names it "customAttribute" (config.attrSeamFn).
function attrSeamFnOf(config) {
  return config.attrSeamFn || "attribute";
}
...
function renderChildElement(spec, config, acc, ctxLabel) {
  ...
  if (HTML_TEXT_TAGS.has(spec.tag)) {
    ...
    const attrSeamFn = attrSeamFnOf(config);
    const attrExprs = Object.entries(spec.attrs ?? {})
      .filter(([k]) => k !== "slot")
      .sort(([a], [b]) => (a < b ? -1 : a > b ? 1 : 0))
      .map(([k, v]) => renderNativeAttr(attrSeam, k, v, attrSeamFn));
  ...
  // attr (filled/weight/…) stays in the attr list. The opaque-Name CONSTRUCTOR
  // (`icon`/`custom`) and the Name constants live in the icons module
  // (ICON_NAMES.module = `M3e.Icon`); the ATTR SETTERS live on the component
  // surface (`comp.module` = `M3e.Component.Icon`) and type-unify because
  // `M3e.Icon.icon : Name -> List (Attr attrs msg) -> …` is polymorphic in
  // `attrs`. Sourced from ICON_NAMES + the facts, never guessed. (Pre-2026-08-17
  // this used `comp.module` for BOTH — correct only while the component module
  // was the flat `M3e.Icon`; the R-025 `.Component.` infix exposed the coupling.)
  if (spec.tag === ICON_NAMES.cemTag) {
    ...
    usedImportsAdd(acc, ICON_NAMES.module); // icon constructor + Name/custom
    ...
    if (iconAttrExprs.length) usedImportsAdd(acc, comp.module); // attr setters (M3e.Component.Icon)
    ...
    return `${ICON_NAMES.module}.${ICON_NAMES.iconFn} ${nameExpr} ${iconAttrList} ${iconChildList}`;
  }
...
function emitIconTableEntry(entry, config) {
  ...
  for (const row of entry.icons) {
    ...
    // Constructor + Name from the icons module (ICON_NAMES.module = M3e.Icon);
    // the `filled` setter from the component surface (comp.module =
    // M3e.Component.Icon), which type-unifies with the polymorphic icon attrs.
    const attrList = row.filled ? `[ ${comp.module}.${filledSetter} True ]` : "[]";
    const example = `${ICON_NAMES.module}.${ICON_NAMES.iconFn} ${nameExpr} ${attrList} []`;
    const usesCustom = nameExpr.includes(`${ICON_NAMES.module}.${ICON_NAMES.customFn} `);
    // Always import the icons module (ctor + Name); import the component surface
    // only when a setter (filled) is actually emitted, to keep imports minimal.
    const iconImports = [`import ${ICON_NAMES.module}`];
    if (row.filled) iconImports.push(`import ${comp.module}`);
    iconImports.sort();

    const headerLines = [
      ` * GENERATED by cem-figma-connect (profiles/m3-kit/emitters/elm.mjs) — do not edit by hand.`,
      ` * ${entry.cemTag} (iconTable row "${row.figmaName}") -> ${ICON_NAMES.module}.${ICON_NAMES.iconFn} ${nameExpr} (opaque-Name icon, R-026).`,
      ` * bound to Figma icon node ${row.figmaNodeId}; symbol "${row.symbolName}"${row.filled ? " (filled)" : ""}.`,
      ...(usesCustom
        ? [
            ` * "${row.symbolName}" has no exposed Name constant — emitted via the ${ICON_NAMES.module}.${ICON_NAMES.customFn} escape hatch (not guessed).`,
          ]
        : []),
      ` * icon Name resolved from the opaque-Name catalog (profiles/m3-kit/facts/icon-names.json), derived from the generated ${ICON_NAMES.module} module; filled setter from the ${comp.module} component surface (Face C).`,
    ];
    ...
    const contents =
      ...
      `export default {\n` +
      `  example: figma.code\`${example}\`,\n` +
      `  imports: [${iconImports.map((s) => JSON.stringify(s)).join(", ")}],\n` +
      `  id: ${JSON.stringify(id)},\n` +
      `  metadata: {\n` +
      `    nestable: true,\n` +
      `  },\n` +
...
export const emitter = {
  ...
  emit(entry, ctx) {
    const config = {
      ...
      // Plain-HTML scaffolding seams (previously never threaded from the profile
      // — these keys existed in profile.json but the config object dropped them,
      // so htmlSeamOf/attrSeamOf/attrSeamFnOf always fell back to their built-in
      // defaults). Now honored so the m3-kit profile's TypedHtml.Unsafe.Attributes
      // .customAttribute spelling actually reaches the emitted snippets.
      htmlSeam: ctx.profile.raw?.elm?.htmlSeam ?? "TypedHtml",
      attrSeam: ctx.profile.raw?.elm?.attrSeam ?? "Native",
      attrSeamFn: ctx.profile.raw?.elm?.attrSeamFn ?? "attribute",
