...
all =
    describe "NoFamilyMemberDrift"
        [ test "family module importing exactly its declared members — no error" <|
            \() ->
                """module M3e.Family.Chip exposing (chip, assist, filter)
...
"""
        ...
        , test "component missing from family — declared member never imported" <|
            \() ->
                """module M3e.Family.Chip exposing (chip, assist)
...
"""
                    ...
                    |> Review.Test.expectErrors
                        [ Review.Test.error
                            ...
                            , details =
                                [ "The family config declares `FilterChip` a member of the `Chip` family, but this module has no `import ...Component.FilterChip`."
...
"""
                    ...
                    |> Review.Test.expectErrors
                        [ Review.Test.error
                            ...
                            , under = "import M3e.Component.NavMenuItemGroup as Group_"
                            }
                        ]
        , test "family module with no matching config entry — out of scope, no error" <|
            \() ->
                """module M3e.Family.Unknown exposing (thing)

import M3e.Component.SomethingElse as SomethingElse_
...
"""
        ...
        , test "non-family module — out of scope, no error" <|
            \() ->
                """module M3e.Component.Chip exposing (component)
