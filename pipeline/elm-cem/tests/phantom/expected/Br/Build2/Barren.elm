module Br.Build2.Barren exposing (Builder, AttrCaps, SlotCaps, Is, Content, ChildAdmittedBy, build, toElement, withClass, withCount, withId, withLabel, withSlot, withStyle, withChild)

{-| The **Barren** family — COMPOSED builders (DAG-rework Task 3 dual-emit).

One module carrying every member's builder surface
(degenerate single-member family — flat, un-prefixed surface),
sourced through `Br.Component2.Barren` (the family façade) rather
than the per-element `Br.Element.*` modules. This is the Shape A
`Build2` cutover; it emits ALONGSIDE the shipped per-element `Br.Build.*`
...
import Br.Component2.Barren as Component
