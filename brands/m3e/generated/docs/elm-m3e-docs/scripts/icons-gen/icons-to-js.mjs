...
export function generate(config, sets) {
  const lines = [
    ...
    // From "@m3e/web/all", NOT "@m3e/web/icon". dist/all.js inlines its own copy of
    // IconRegistry and dist/icon.js ships a second, independent one. docs/index.ts
    // loads @m3e/web/all, so that bundle's registry is the one the live <m3e-icon>
    // reads. Registering through @m3e/web/icon writes to the other instance: no
    // error, no warning, and the element still renders its text fallback (verified
    // in a browser — the app bar showed the literal word "github"). @m3e/web/all
    // re-exports the same registerIcon, so this is still the library's public seam.
    'import { registerIcon } from "@m3e/web/all";',
    "",
  ];
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const out = resolve(REPO, "..", "..", "docs", "elm-m3e-docs", "gen", "icons.js");
