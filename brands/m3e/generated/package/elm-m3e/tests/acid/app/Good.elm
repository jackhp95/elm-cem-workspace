...
keep compiling. Exercises, in one value each:
  ...
  - **List form + slot admittance the right way round.** `M3e.Element.AppBar.leading`
  ...
  - **Shared atoms.** `M3e.text` produces `sharedText` and `M3e.icon` produces
    ...
    that name them.
  - **Value narrowing.** `M3e.Element.AppBar.size` takes `Value Size` where
...
view =
    M3e.toHtml
        (M3e.appBar
            [ M3e.Element.AppBar.size M3e.Values.small
            , M3e.Element.AppBar.centered True
            ]
            [ M3e.Element.AppBar.leading (M3e.icon [] [])
            , M3e.Element.AppBar.title (M3e.heading [] [ M3e.text "Inbox" ])
            , M3e.Element.AppBar.trailing (M3e.iconButton [] [ M3e.icon [] [] ])
            , M3e.Element.AppBar.trailing (M3e.searchBar [] [])
            , M3e.Element.AppBar.trailing (M3e.Unsafe.recast (M3e.chip [] [ M3e.text "All" ]))
            ]
        )


bar : M3e.Element (M3e.Element.AppBar.Is s) admittedBy msg
bar =
    AppBarB.build
