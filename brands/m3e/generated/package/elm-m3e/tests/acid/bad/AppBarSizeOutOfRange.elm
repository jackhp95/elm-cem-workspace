...
components, and each component closes the set it accepts. `M3e.Element.AppBar.size`
takes `Value M3e.Element.AppBar.Size` where
...
positive half lives in `app/Good.elm`: `M3e.Element.AppBar.size M3e.Values.small`
...
broken =
    M3e.appBar [ M3e.Element.AppBar.size M3e.Values.extraLarge ] []
