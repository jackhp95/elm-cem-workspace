module M3e.Element.Tooltip exposing
    ( component
...
```elm
[ M3e.Element.IconButton.component { content = M3e.Element.Icon.component [ M3e.Element.Icon.name "arrow_back" ] [], ariaLabel = "Go back", action = M3e.Action.none } [ M3e.Attributes.id "button" ] []
    , M3e.Element.Tooltip.component { content = M3e.text "Go Back" } [ M3e.Element.Tooltip.for "button" ] []
    ]
```

<!-- elm-cem:example title="Rich tooltip" -->
```elm
[ M3e.Element.IconButton.component { content = M3e.Element.Icon.component [ M3e.Element.Icon.name "settings" ] [], ariaLabel = "Settings", action = M3e.Action.none } [ M3e.Attributes.id "button2" ] []
    , M3e.Element.RichTooltip.component { content = M3e.text "Now you can adjust the uploaded image quality, and upgrade your available storage space." } [ M3e.Element.RichTooltip.for "button2" ] [ M3e.Element.RichTooltip.subhead (M3e.text "New settings available") ]
    ]
```

<!-- elm-cem:example title="Actions" -->
```elm
[ M3e.Element.IconButton.component { content = M3e.Element.Icon.component [ M3e.Element.Icon.name "settings" ] [], ariaLabel = "Settings", action = M3e.Action.none } [ M3e.Attributes.id "button3" ] []
    , M3e.Element.RichTooltip.component { content = M3e.text "Now you can adjust the uploaded image quality, and upgrade your available storage space." } [ M3e.Element.RichTooltip.for "button3" ] [ M3e.Element.RichTooltip.subhead (M3e.text "New settings available"), M3e.Element.RichTooltip.actions (TypedHtml.div [] [ M3e.Element.Button.component { content = M3e.Element.RichTooltipAction.component { content = M3e.text "Learn more" } [] [], action = M3e.Action.none } [] [] ]) ]
    ]
```
